...
def define_ds_dl_join(doc_dict_folder, data_type_list, ds_list_folder, lim,
    ...
    # Add warning if we are using lim != None
    if lim != None:
    ...
    ds_test = \
        GraphSiameseDatasetDictJoin(ds_list_test,
                                    ...
                                    [_tsfm_dict[da] for da in
                                     data_available_list],
                                    lim=lim)
    print(f'\nNumber of training graphs: {len(ds_train)}', file=f)
...
def run_several_experiments(dataset_name, folder_sufix, dest_folder_prefix,
    ...
    for i, exp_op in enumerate(exp_ops):
        ...
        for j in range(repeat_experiment):
            ...
            # Add number of instances in dataset
            print(f'Number of training graphs: {len(dl_train.dataset)}', file=f)
            print(f'Number of val graphs: {len(dl_val.dataset)}', file=f)
            print(f'Number of test graphs: {len(dl_test.dataset)}\n', file=f)
...
def continue_training():
    # TODO: write as a function, add cli
        # dataset_name, folder_sufix, dest_folder_prefix,
                      # doc_dict_folder_prefix, ds_list_folder_prefix, lim,
                      # batch_size, num_workers,
                      # epochs, device, main_loss,
                      # bm_free_epochs,
                      # checkpoint_freq,
                      # model_type, data_type_list, base_path, cont_ref,
                      # model_path):
    """Function to continue training some model. We can use this function for
    transfer learning.

    Based on siamese_graph_trainer.continue_training in tesis_maestría
    repository
    """

    # # ========== Test parameters init common
    # dataset_name = '20-small-bal'
    # folder_sufix = '_None_2000'
    # dest_folder_prefix = '../test/test_output_siamese/test_continue/'
    ...
    # # ========== Test parameters legacy GBSN single
    # # ========== Datasets options
    # # model_type = 'med'
    # # graph_type = model_type
    # # data_type_list = [graph_type]

    # # # ========== Model
    # # base_path = ('../test/sample_trained_models/legacy_classes/'
                 # # '20-large-train_05090350')
    # # cont_ref = base_path[-8:]
    # # model_path = glob.glob(os.path.join(base_path,
                                        # # graph_type + '*/checkpoint_last*'))[0]

    # # ========== Test parameters legacy GBSN ensemble
    # # ========== Datasets options
    # model_type = 'ensemble'
    # data_type_list = ['short',
                      ...
                      # 'text_features']
    # # ========== Model
    # base_path = '../PAN21_predict/small_join_05171403_best_model_'
    # cont_ref = '05171403'
    ...
    data_type_list = ['short',
                      ...
                      'text_features']
    # ========== Model
    base_path = '../PAN21_predict/large_join_05181529_best_model_'
    cont_ref = '05171403'
    model_path = base_path + '.pth'
    ...
    model_dict = load_checkpoint(model_path, opt_class=opt_class,
                                 device=device)
    model = model_dict['model']
    opt = model_dict['optimizer']
    loss_init = model_dict['loss']
    epoch_init = model_dict['epoch'] + 1
    model_class = model_dict['model_class']
    model_args = model_dict['model_args']

    exp_op = {'base_path': base_path,
              'model_class': model_dict['model_class'],
              'model_args': model_dict['model_args']}
    # optimizer
    # opt = torch.optim.Adam(model.parameters(), lr=lr)
    # ========== Setup log file (datasets and experiment)
    run_name = 'cont(' + cont_ref + ')-' + model_type + '_' + time_stamp
    log_path = os.path.join(dest_folder_exp, run_name + '_log.txt')
    print('log save in: ' + log_path)
    f = open(log_path, 'w+')
    print(f'Continue training {cont_ref}. epoch_init {epoch_init}', file=f)
    print(f'Continue training {cont_ref}. epoch_init {epoch_init}')
    # ========== Define dataset and dataloaders
    dl_train, dl_val, dl_test = \
        define_ds_dl_join(doc_dict_folder, data_type_list,
                          ds_list_folder, lim,
